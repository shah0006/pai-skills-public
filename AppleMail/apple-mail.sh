...
create_mailbox() {
    ...
    if [[ $rc -ne 0 ]] && $is_imap; then
        # AppleScript failed on IMAP account -- try gws CLI fallback for Gmail
        if [[ "$acct" == "Google" ]] && command -v gws &>/dev/null; then
            local gws_result
            gws_result=$(gws gmail users labels create --json "{\"name\": \"${mb}\"}" 2>&1)
            if [[ $? -eq 0 ]]; then
                echo "Created (via Gmail API): ${path}"
                return 0
            else
                echo "Error: AppleScript and Gmail API both failed for '${path}'" >&2
                echo "  AppleScript: ${result}" >&2
                echo "  Gmail API: ${gws_result}" >&2
                return 1
            fi
        fi
        echo "$result" >&2
...
WATCH_SCRIPT="$(cd "$(dirname "$0")" && pwd)/watch-check.sh"
WATCH_VIP="$(cd "$(dirname "$0")" && pwd)/watch-vip.txt"
...
main() {
    ...
    case "${command}" in
        ...
        create-mailbox|create-mb)
            [[ $# -eq 0 ]] && { echo "Error: Unified path required" >&2; exit 1; }
            create_mailbox "$@" ;;
        delete-mailbox|delete-mb)
            [[ $# -eq 0 ]] && { echo "Error: Unified path required" >&2; exit 1; }
            delete_mailbox "$@" ;;
        rename-mailbox|rename-mb)
